...
public class AnimatedBone {
    ...
    private int getPositionKeyInx(float animationTime) {
        for (int i = 0; i < keyPositions.size(); i++) {
            if (animationTime < keyPositions.get(i).timestamp) return i;
        }
        return 0;
    }
    ...
    private int getRotationKeyInx(float animationTime) {
        for (int i = 0; i < keyRotations.size(); i++) {
            if (animationTime < keyRotations.get(i).timestamp) return i;
        }
        return 0;
    }

    private int getScaleKeyInx(float animationTime) {
        for (int i = 0; i < keyScales.size(); i++) {
            if (animationTime < keyScales.get(i).timestamp) return i;
        }
        return 0;
    }
...
//        int nextKeyInx = (currentKeyInx + 1) % keyPositions.size();  // todo: this?
        ...
        Vector3f position = keyPositions.get(currentKeyInx).position.lerp(keyPositions.get(nextKeyInx).position, percent);
    ...
    private Matrix4f interpolateRotation(float animationTime) {
        ...
        Quaternionf rotation = keyRotations.get(currentKeyInx).orientation.slerp(keyRotations.get(nextKeyInx).orientation, percent);
    ...
    private Matrix4f interpolateScale(float animationTime) {
        ...
        Vector3f scale = keyScales.get(currentKeyInx).scale.lerp(keyScales.get(nextKeyInx).scale, percent);
